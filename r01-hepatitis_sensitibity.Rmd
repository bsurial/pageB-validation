---
title: "Page-B in HIV/HBV coinfection - Exclude HCV and HDV"
description: |
...
# REMOVE patients with hepatitis D or C
time_sub %>% 
  count(hcv_coinfection, 
        any_hdv) %>% 
  group_by(hcv_coinfection) %>% 
  mutate(sum = sum(n))
  
# 274 with HCV (39 had HDV + HCV) and 108 with HDV = 382

time_sub <- time_sub %>% 
  filter(hcv_coinfection == 0, 
         any_hdv == 0 | is.na(any_hdv))
...
estimate <- summary(fit, times = c(1, 3, 5, 10, 15), fun = "event")
...
Aquitaine, and `r n_hcc("SHCS")` in the SHCS. The cumulative 1-, 3-, 5-, 10-, 
and 15-year rates of HCC were
...
ggsave(plot = kaplan_15y_cc, here("graphs", "r01-kaplan-meier_15y.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
```
...
write_rds(imp_data, here("processed", "r01-imputed_data.rds"))
```

```{r read-impute-data, eval=!params$impute}
imp_data <- read_rds(here("processed", "r01-imputed_data.rds"))
```
...
# Assess imputation
png(file = here("graphs", "r01-imputation_density.png"),
...
ggsave(here("graphs", "r01-kaplan_meyer_15y_imp.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
...
ggsave(plot = p_combined_h, 
       filename = here("graphs", "r01-kaplan_combined.png"), 
       dpi = 300, bg = "white", width = 10, height = 5)

ggsave(plot = p_combined_h, 
       filename = here("graphs", "r01-kaplan_combined.pdf"), 
       dpi = 300, bg = "white", width = 10, height = 5, 
       device = cairo_pdf)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "r01-kaplan_combined_v.png"), 
       dpi = 300, bg = "white", width = 8, height = 10)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "r01-kaplan_combined_v.pdf"), 
       dpi = 300, bg = "white", width = 8, height = 10, 
       device = cairo_pdf)


ggsave(plot = pp$table, 
       filename = here("graphs", "r01-kaplan_risktable.pdf"),
       dpi = 300, device = cairo_pdf, width = 8, height = 3)
```
...
```{r write-out}
write_rds(time_sub, here("processed", "r01-studypop.rds"))
write_rds(imp_long, here("processed", "r01-imp_long_df.rds"))
```
