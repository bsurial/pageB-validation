---
title: "Page-B in HIV/HBV coinfection (No African)"
description: |
...
time_dat <- full %>% 
  ...
  mutate(age = year(tfv_sd) - birth_d, 
         ...
         plt_c = case_when(plt >= 200 ~ "200+", 
                           plt >= 100 & plt < 200 ~ "100-199", 
                           plt < 100 ~ "<100"), 
...
african <- time_dat %>% 
  filter(origin_cat %in% c("African")) %>% nrow()
...
time_sub <- time_dat %>% 
  filter(time_y2 > 0) %>% 
  filter(origin_cat != "African")

n_study <- nrow(time_sub)
```
...
vars <- c("male", "age", "caucasian", "origin_cat", "transmission_risk", "prior_xtc", 
          "prior_xtc_years", "plt", "plt_c", "hbe_baseline", "any_hdv", 
...
`r estimate$std.error`). The slope is lower than 1, which indicates
impaired prediction, however the p-value is not significant (*P* =
`r estimate$p.value`).
...
cph(S ~ male + age + plt_c + offset(LP), data = time_sub)
...
ggsave(plot = kaplan_15y_cc, here("graphs", "08a-kaplan-meier_15y_noafrican.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
```
...
sub <- time_sub %>%
  select(p_id, c_id, plt, gender, tfv_sd, cohort, birth_d, caucasian, height, death,
         drop, transmission_risk, co_dia, hcv_coinfection, alb_baseline, any_hdv,
         cd4_baseline, apri_baseline,prior_xtc, prior_xtc_years,
         bil_baseline, hcc, hcc_d, last_fup_d, delta_fup, delta_hcc, 
         event, event2, time_y, time_y2, cirrhosis, cirrhosis_basis)
...
write_rds(imp_data, here("processed", "08a-imputed_data_no_african.rds"))
```

```{r read-impute-data, eval=!params$impute}
imp_data <- read_rds(here("processed", "08a-imputed_data_no_african.rds"))
```
...
to have worked since the distribution of platelet values (`plt`) follows
...
# Assess imputation
png(file = here("graphs", "08a-imputation_density_no_african.png"),
    res = 300, width = 19, height = 16, unit = "cm")
densityplot(imp_data, data = ~ plt | cohort,
...
imp_long %>% 
  ...
  knitr::kable(digits = 1, caption = "Distribution of PAGE-B scores")
...
imp_long %>% 
  ...
  group_by(hcc) %>% 
  mutate(p = n/sum(n) * 100) %>% 
  knitr::kable(digits = 1, caption = "Distribution of HCC cases")
...
imp_results1 <- tibble(estimate) %>% rename(c.low = "2.5 %", 
                                            c.hi = "97.5 %")
```
...
ggsave(here("graphs", "08a-kaplan_meyer_15y_imp_noafrican.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
...
ggsave(plot = p_combined_h, 
       filename = here("graphs", "08a-kaplan_combined_noafrican.png"), 
       dpi = 300, bg = "white", width = 10, height = 5)

ggsave(plot = p_combined_h, 
       filename = here("graphs", "08a-kaplan_combined_noafrican.pdf"), 
       dpi = 300, bg = "white", width = 10, height = 5, 
       device = cairo_pdf)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08a-kaplan_combined_v_noafrican.png"), 
       dpi = 300, bg = "white", width = 8, height = 10)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08a-kaplan_combined_v_noafrican.pdf"), 
       dpi = 300, bg = "white", width = 8, height = 10, 
       device = cairo_pdf)

ggsave(plot = pp$table, 
       filename = here("graphs", "08a-kaplan_combined_risktable.pdf"), 
       dpi = 300, bg = "white", width = 8, height = 3, 
       device = cairo_pdf)
...
```{r write-out}
write_rds(time_sub, here("processed", "08a-studypop_noafrican.rds"))
write_rds(imp_long, here("processed", "08a-imp_long_df_noafrican.rds"))
```
