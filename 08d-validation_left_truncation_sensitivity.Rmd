---
title: "Page-B in HIV/HBV coinfection - Left-Truncation Analysis"
description: |
...
time_dat <- full %>% 
  ...
  mutate(age = year(tfv_sd) - birth_d, 
         ...
         # Left_truncation
         left_c = tfv_sd < regdate, 
         time0 = if_else(left_c == FALSE, 0, 
                         as.numeric(regdate - tfv_sd)/365.25)) %>% 
  # Now calculate fup
  mutate(delta_hcc = as.numeric(hcc_d - tfv_sd) / 365.25, 
         delta_fup = as.numeric(last_fup_d - tfv_sd) / 365.25) %>% 
...
n_study <- nrow(time_sub) # 2963 in sensitivity analysis. 
```
...
This analysis is done after input from Andi. There are a few patients 
how enter the cohort (regdate) after TFV start. As we are not sure how well
HCC diagnosis before regdate were registered. Time-to-event is always counted 
from TFV start, but fup only starts to count at regdate. This is accomplished 
with `Surv(time = tfv_sd or regdate, time2 = event_d - tfv_sd for all)`. 

However, I wouldn't use this analysis as the "Main" one because there are at
least 4 patients who developed HCC date before cohort entry (3 in Eurosida) and
we have data on them. So excluding them seems wrong to me. 
...
```{r hcc-incidence}
fit <- survfit(Surv(time = time0, time2 = time_y2, event2) ~ 1, data = time_sub)
...
S <- Surv(event = time_sub$event2, time = time_sub$time0, time2 = time_sub$time_y2)
...
S_kaplan <- survfit(Surv(time = time0, time2 = time_y, event = event) ~ page_b_cat, time_sub) 
...
S_kaplan <- survfit(Surv(time = time0, time2 = time_y2, event = event2) ~ page_b_cat, time_sub) 
...
sub <- time_sub %>%
  select(p_id, c_id, plt_2, gender, tfv_sd, cohort, birth_d, caucasian, 
         height, death,
         ...
         event, event2, time0, time_y, time_y2)
...
predM[, c("hcc_d", "last_fup_d", "delta_fup", "delta_hcc", "time0",
          "time_y", "event", "event2", "cirrhosis_basis")] <- 0
meth[c("hcc_d", "last_fup_d", "delta_fup", "delta_hcc", "time0",
...
write_rds(imp_data, here("processed", "08d-imputed_data_left_trunc.rds"))
```

```{r read-impute-data, eval=!params$impute}
imp_data <- read_rds(here("processed", "08d-imputed_data_left_trunc.rds"))
```
...
# Assess imputation
png(file = here("graphs", "08d-imputation_density.png"),
...
model <- with(imp_mids, coxph(Surv(event = event, time = time0, time2 = time_y) ~ LP))
...
model <- with(imp_mids, coxph(Surv(event = event, time0, time2 = time_y) ~ LP + offset(LP)))
pm <- pool(model) %>% summary(conf.int = TRUE)
...
model2 <- with(imp_mids, 
               coxph(Surv(event = event, time = time0, time2 = time_y) ~ 
                       male + age + plt_c + offset(LP)))

model3 <- with(imp_mids, 
               coxph(Surv(event = event, time = time0, time2 = time_y) ~ 1))
# Joint test of 0
...
S_kaplan <- survfit(Surv(time = time0, time2 = time_y, 
                         event = event) ~ page_b_cat, df_kaplan) 
pp <- ggsurvplot(S_kaplan, 
                fun = "cumhaz", 
                conf.int = FALSE, 
                pval = FALSE, 
...
# Step 1:
model <- with(imp_mids, coxph(Surv(event = event2, 
                                   time = time0, time2 = time_y2) ~ LP))
pm <- pool(model) %>% summary(conf.int = TRUE)
...
model <- with(imp_mids, coxph(Surv(event = event2, 
                                   time = time0, time2 = time_y2) ~ LP + offset(LP)))
...
# Step 2: 
model2 <- with(imp_mids, 
               coxph(Surv(event = event2, 
                          time = time0, time2 = time_y2) ~ male + age + plt_c + offset(LP)))

model3 <- with(imp_mids, 
               coxph(Surv(event = event2, 
                          time = time0, time2 = time_y2) ~ 1))
# Joint test of 0
a <- suppressWarnings(D1(model2, model3))$result
...
S_kaplan <- survfit(Surv(time = time0, 
                         time2 = time_y2, event = event2) ~ page_b_cat, df_kaplan) 
pp <- ggsurvplot(S_kaplan, 
           fun = "cumhaz", 
           conf.int = FALSE, 
           pval = FALSE, 
...
ggsave(here("graphs", "08d-kaplan_meyer_15y_imp_left_c.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
...
ggsave(plot = p_combined_h, 
       filename = here("graphs", "08d-kaplan_combined_left_c.png"), 
       dpi = 300, bg = "white", width = 10, height = 5)

ggsave(plot = p_combined_h, 
       filename = here("graphs", "08d-kaplan_combined_left_c.pdf"), 
       dpi = 300, bg = "white", width = 10, height = 5, 
       device = cairo_pdf)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08d-kaplan_combined_v_left_c.png"), 
       dpi = 300, bg = "white", width = 8, height = 10)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08d-kaplan_combined_v_left_c.pdf"), 
       dpi = 300, bg = "white", width = 8, height = 10, 
       device = cairo_pdf)
```
...
x_comp <- survfit(Surv(time = time0, time2 = time_y2, event = event2) ~ page_b_cat, 
                  time_sub) %>% 
...
```{r write-out}
write_rds(time_sub, here("processed", "08d-studypop_sens_left_c.rds"))
write_rds(imp_long, here("processed", "08d-imp_long_df_sens_left_c.rds"))
```
