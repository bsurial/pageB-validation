---
title: "Page-B in HIV/HBV coinfection - Sensitivity ONLY AFTER COHORT REG"
description: |
...
time_dat %>% 
  group_by(cohort) %>% 
  count(prior = regdate > tfv_sd) %>% 
  mutate(p = n / sum(n)) %>% 
  filter(prior == TRUE) %>% 
  ungroup() %>% 
  mutate(total = sum(n)) 
# 715 started tenofovir before cohort entry:
# Aquitaine 94 (28%),ATHENA 243 (18%), Eurosida 284 (35%), SHCS 94 (18%)
...
# Reomve individuals who started TFV before Registering in the Cohort
time_sub <- time_sub %>% 
  filter(regdate <= tfv_sd) 


n_study <- nrow(time_sub) # 2273
```
...
ggsave(plot = kaplan_15y_cc, here("graphs", "08c-kaplan-meier_15y_sens2.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
```
...
write_rds(imp_data, here("processed", "08c-imputed_data_sens2.rds"))
```

```{r read-impute-data, eval=!params$impute}
imp_data <- read_rds(here("processed", "08c-imputed_data_sens2.rds"))
```
...
# Assess imputation
png(file = here("graphs", "08c-imputation_density_sens2.png"),
...
ggsave(here("graphs", "08c-kaplan_meyer_15y_imp_sens.png"), 
       dpi = 300, bg = "white", 
       width = 6, height = 5)
...
ggsave(plot = p_combined_h, 
       filename = here("graphs", "08c-kaplan_combined_sens.png"), 
       dpi = 300, bg = "white", width = 10, height = 5)

ggsave(plot = p_combined_h, 
       filename = here("graphs", "08c-kaplan_combined_sens.pdf"), 
       dpi = 300, bg = "white", width = 10, height = 5, 
       device = cairo_pdf)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08c-kaplan_combined_v_sens.png"), 
       dpi = 300, bg = "white", width = 8, height = 10)

ggsave(plot = p_combined_v, 
       filename = here("graphs", "08c-kaplan_combined_v_sens.pdf"), 
       dpi = 300, bg = "white", width = 8, height = 10, 
       device = cairo_pdf)
```
...
```{r write-out}
write_rds(time_sub, here("processed", "08c-studypop_sens.rds"))
write_rds(imp_long, here("processed", "08c-imp_long_df_sens.rds"))
```
